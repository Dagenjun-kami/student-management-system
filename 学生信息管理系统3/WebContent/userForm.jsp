	...
	<div class="divcenter">
		<form action="userForm.jsp"
			...
			<input id="name" name="name" placeholder="按姓名查询"> <input
				...
				onclick="window.location.href='userAddForm.jsp'">
		</form>
		<table class="table table-hover">
			...
			<%
				...
				List<User> lst = sdb.getUser(sql);
				request.setAttribute("userLst", lst);
			%>
			<c:forEach items="${userLst}" var="user">
				<tr id="${user.sid}">
					<td>${user.sid}</td>
					<td>${user.sname}</td>
					<td>${user.spassword}</td>
					<td>${user.sauthority}</td>
					<td><a href="userDel.jsp?id=${user.sid}"
						style="text-decoration: none">删除</a></td>
					<td><a
						href="userModForm.jsp?id=${user.sid}&name=${user.sname}&password=${user.spassword}&authority=${user.sauthority}"
